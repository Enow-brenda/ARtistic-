...
$aid=$_GET['id'];
    ...
    $rs = $conn->query($query);
    $row = $rs->fetch_assoc();
    $query1="SELECT * FROM `admin` where adminid='$aid' ";
...
<html lang="en">
  <head>
    ...
    <title>ARtistic - Dashboard</title>
...
<!-- content inside the main-->
    ...
    <?php if(isset($_GET['error'])){?>
      ...
      <div class="r pt-3 pb-2 mb-3 ">
        ...
        <div>
        <a href="admin.php" class="btn d-inline-flex btn-sm btn-danger border-base mx-1">
                                    <span class=" pe-2">
        ...
        <div class="card shadow border-0 mb-7" style="margin-top:30px;">
                   ...
                   <label>Secret Key</label>
                    ...
                    <label>Contact</label>
                    <input type="number" name="contact" value="<?php echo $row1['contact']?>" class="form-control mb-2">
                    <label>Gender</label>
                    <select name="gender" value="<?php echo $row1['gender']?>" class="form-control mb-2">
                    ...
                    <?php if($row1['class']=='A'){?>
                        ...
                        <?php }elseif($row1['class']=='B'){?>
                        <option  value="A">A</option>
                        <option selected value="B">B</option>
                        <option value="C">C</option>
                        <?php }elseif($row1['class']=='C'){?>
                        <option  value="A">A</option>
                        <option value="B">B</option>
                    ...
                    <input type="text" value="<?php echo $row1['position']?>" name="position" class="form-control mb-2">
                    <label>Admin Image</label>
                    <input type="file" name="profilepic" class="form-control mb-2 me-2" multiple accept="image/*">
                    <button class="btn btn-primary" name="updadmin" style="background-color: #ff6b6b ;border:none;" type="submit">Edit Admin</button>
                   </form>
                </div>
