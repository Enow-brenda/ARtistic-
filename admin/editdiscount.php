...
$cid=$_GET['id'];
    ...
    $rs = $conn->query($query);
    $row = $rs->fetch_assoc();
    $query1="SELECT * FROM `coupon` where id='$cid' ";
...
<!-- content inside the main-->
    ...
    <?php if(isset($_GET['error'])){?>
      ...
      <div class="r pt-3 pb-2 mb-3 ">
        ...
        <div>
        <a href="discount.php" class="btn d-inline-flex btn-sm btn-danger border-base mx-1">
                                    <span class=" pe-2">
        ...
        <div class="card shadow border-0 mb-7" style="margin-top:30px;">
                    <div class="card-header">
                        <h5 class="mb-0">Add Disount</h5>
                    </div>
                   <form class="p-4 pt-3" action="operation.php" method="post" enctype="multipart/form-data">
                    <input type="hidden"name="id" value="<?php echo $cid?>">
                    <label>coupon</label>
                    <input type="text" name="coupon" class="form-control mb-2" value="<?php echo $row1['coupon'];?>" required> 
                    <label>Discount Percentage</label>
                    <input type="number" name="percent" value="<?php echo $row1['discountpercent'];?>" class="form-control mb-2">
                    <label>Start Date</label>
                    <input type="date" name="start" value="<?php echo $row1['startdate'];?>" class="form-control mb-2" required>
                    <label>End Date</label>
                    <input type="date" name="end" value="<?php echo $row1['enddate'];?>" class="form-control mb-2" required>
                    
                   
                    <button class="btn btn-primary" style="background-color: #ff6b6b ;border:none;" name="editdiscount" type="submit">Edit Disount</button>
                   </form>
                </div>
